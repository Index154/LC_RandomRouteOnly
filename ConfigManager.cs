...
public class ConfigManager {
    ...
    internal void Setup(ConfigFile configFile) {
        rerollsPerPlayer = configFile.Bind("1. General", "Rerolls per player per quota", 1, new ConfigDescription("How many rerolls each player gets on every quota"));

        allowCompany = configFile.Bind("1. General", "Allow manually routing to Company", false, new ConfigDescription("If true, allows you to always route to the Company whenever you want"));

        stayOnMoonDays = configFile.Bind("1. General", "Max consecutive days per moon", 1, new ConfigDescription("The number of days you can spend on one moon before the ship automatically travels to a new random moon. If you route to the Company before reaching this limit then the ship will return to the previous moon after leaving the Company. Consecutive days per moon are not tracked across game restarts", new AcceptableValueRange<int>(1, 100), Array.Empty<object>()));
        ...
        constellations = configFile.Bind("2. Other", "Constellations compatibility", true, new ConfigDescription("Whether random routing should only select moons in the current constellations if LethalConstellations is enabled"));
    }
}